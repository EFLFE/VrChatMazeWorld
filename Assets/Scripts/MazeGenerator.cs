...
public class MazeGenerator : UdonSharpBehaviour {
    ...
    public void GenerateFinish() {
        ...
        // generate second floors for Square rooms
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                for (int z = 1; z < height; z++) {
                    if (
                        ids[x][y][z] == 0 && // текущая ячейка не комната
                        cells[x][y][z - 1] != Cell.Ceiling && // ячейка ниже не потолок
                        ids[x][y][z - 1] > 0 && // ячейка ниже комната
                        rooms[ids[x][y][z - 1]] == Room.Square && // ячейка ниже из квадратной комнаты
                        ids[x][y][z - 1] % 2 == 0 // комната чётная
                    ) {
                        // вставить второй этаж типа "потолок"
                        ids[x][y][z] = ids[x][y][z - 1];
                        cells[x][y][z] = Cell.Ceiling;
                    }
                }
            }
        }
