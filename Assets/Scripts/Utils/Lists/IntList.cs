...
public class IntList : UdonSharpBehaviour {
    public int Count => _count;

    private int[] _storage;
    private int _count;

    public int[] Items => _storage;

    public void Init(int initialCapacity) {
        _storage = new int[initialCapacity];
    }
    ...
    private void ExpandCapacity(int extraCapacity) {
        ...
        var newBuffer = new int[_storage.Length + extraCapacity];
    ...
    public void Swap(int firstIndex, int secondIndex) {
        ...
        var firstItem = _storage[firstIndex];
    ...
    public void Clear(bool shrinkBuffer = false) {
        ...
        _count = 0;
        if (shrinkBuffer) {
            _storage = new int[ListManager.MIN_CAPACITY_EXPAND_SIZE];
        }
    }
