...
public class MazeBuilder : UdonSharpBehaviour {
    ...
    [SerializeField] private Mesh[] walls_solid;
    [SerializeField] private Mesh[] walls_holed;
    ...
    [Header("Enemies")]
    [SerializeField] private PoolObjects[] enemiesPool;
    ...
    private bool SpawnCell(int x, int y, int z) {
        ...
        for (int direction = 1; direction <= 4; direction++) {
            ...
            if (current_cell == Cell.DoorDeadEnd && neighbor == Cell.Wall) {
                ...
                obj.name = $"id={ids[x][y][z]}, deadends, variant {deadend_variant}, {debug}";
            } else if (neighbor == Cell.Wall || nearId == 0) {
                // spawn wall
                obj = SpawnWithOffsetAndRecolor(walls_solid[GetRandomIndexOfWall(walls_solid.Length)], x, y, z, rotation);
                obj.name = $"id={ids[x][y][z]}, walls_solid, {debug}";
                ...
                if (
                    ...
                    obj.name = $"id={ids[x][y][z]}, doors, {debug}";
                } else {
                    if (current_id > 0) {
                        obj = SpawnWithOffsetAndRecolor(walls_holed[GetRandomIndexOfWall(walls_holed.Length)], x, y, z, rotation);
                        obj.name = $"id={ids[x][y][z]}, walls_holed, {debug}";
                    } else {
                        obj = SpawnWithOffsetAndRecolor(walls_solid[GetRandomIndexOfWall(walls_solid.Length)], x, y, z, rotation);
                        obj.name = $"id={ids[x][y][z]}, walls_solid, {debug}";
                    }
        ...
        // спавн противников
        if (current_cell == Cell.Enemy && enemiesPool.Length > 0) {
            int enemy_random_index = RandomInclusive(0, enemiesPool.Length - 1);
            if (!enemiesPool[enemy_random_index].TryTake(out MazeObject mazeObject, ConvertPositionToUnitySpace(new Vector3Int(x, y, z)), Quaternion.identity)) {
                Debug.LogError("No more skelets!");
            }
        }
    ...
    private int GetRandomIndexOfWall(int length) {
        if (RandomInclusive(0, 100) / 100.0f < 0.5f) {
            return RandomInclusive(0, walls_simple_count - 1);
        } else {
            return RandomInclusive(walls_simple_count, length - 1);
        }
    }
    ...
    private GameObject SpawnWithRecolor(
        ...
        material.SetFloat("_Hue", ((controller.level + 2) * 0.31473248f) % 1f);
        material.SetFloat("_Contrast", ((controller.level + 2) * 374262944 % 5 + 8) / 10f);
    ...
    private MazeObject SpawnChest(int x, int y, int z) {
        ...
        controller.MazeUI.UILog(
            $"Spawn {GO.name}, id = {treasure.PoolID} " +
