...
public class MazeBuilder : UdonSharpBehaviour {
    ...
    private bool SpawnCell(int x, int y, int z) {
        ...
        // спавн противников
        if (current_cell == Cell.Enemy && enemies.Length > 0) {
            int enemy_random_index = RandomInclusive(0, enemies.Length - 1);
            //SpawnEnemy(enemies[enemy_random_index], x, y, z);
            GameObject enemy_go = Instantiate(enemies[enemy_random_index], mazeContainer);
            enemy_go.transform.SetPositionAndRotation(
                ConvertPositionToUnitySpace(new Vector3Int(x, y, z)),
                Quaternion.Euler(0, 0, 0)
            );
            // TODO тут можно добавить айдишник комнаты противнику
            // enemy_go.GetComponent<MazeEnemy>().SetRoomID(current_id);
        }
